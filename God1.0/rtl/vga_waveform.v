...
//   95.88kHz 采样, 2048点循环缓冲 (21.3ms), 描点连线
...
// 2048 点缓冲 = 21.3ms @95.88kHz (cap_timer=20ms 安全)
localparam BUF_SIZE = 2048;
(* ramstyle = "M9K" *) reg [7:0] wave_buf [0:BUF_SIZE-1];
integer i;
initial begin
...
always @(posedge vga_clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (snap)
            disp_origin <= wr_ptr;
        if (ad_data_vld && !freeze) begin
            wave_buf[wr_ptr] <= {~ad_data[23], ad_data[22:16]};  // 24bit补码→8bit无符号: 符号位bit[23]取反+高7位bit[22:16]
            wr_ptr <= wr_ptr + 1'b1;
        end
    end
end
...
reg [7:0]  sample;
reg [9:0]  pix_x_d1, pix_y_d1;
reg        in_wave_d1;
reg [10:0] x_buf_d1;
reg        x_buf_chg, x_buf_chg_d1;
reg [9:0]  wave_y, wave_y_prev;
...
always @(posedge vga_clk) begin
    ...
    x_buf_d1   <= x_buf;
    x_buf_chg    <= (x_buf != x_buf_d1);
    x_buf_chg_d1 <= x_buf_chg;
    wave_y      <= 10'd428 - {2'b0, sample};
    if (x_buf != x_buf_d1)
        wave_y_prev <= 10'd428 - {2'b0, sample};
end

// 波形命中 (display_en=0 时不显示波形, 仅坐标轴)
wire on_wave = display_en && in_wave_d1 && (pix_y_d1 >= wave_y - 1) && (pix_y_d1 <= wave_y + 1);
wire line_active = x_buf_chg || x_buf_chg_d1;
wire on_line = display_en && in_wave_d1 && line_active &&
    ((pix_y_d1 >= wave_y && pix_y_d1 <= wave_y_prev) ||
     (pix_y_d1 >= wave_y_prev && pix_y_d1 <= wave_y));

// UI
wire on_y_axis = (pix_x_d1 == X_LEFT) && (pix_y_d1 >= Y_TOP) && (pix_y_d1 <= Y_BTM);
wire on_x_axis = (pix_y_d1 == Y_MID) && (pix_x_d1 >= X_LEFT) && (pix_x_d1 <= X_RIGHT);
wire on_y_ticks = (pix_x_d1 >= X_LEFT - 3) && (pix_x_d1 <= X_LEFT) &&
                   ((pix_y_d1 == Y_MID) || (pix_y_d1 == Y_TOP + 135) || (pix_y_d1 == Y_BTM - 135));
// X轴刻度: 主刻度 128px(2ms), 小刻度 32px(500us)
wire [9:0] xrel = (pix_x_d1 >= X_LEFT) ? (pix_x_d1 - X_LEFT) : 10'd1000;
wire on_x_major = (pix_y_d1 >= Y_MID) && (pix_y_d1 <= Y_MID + 4) &&
                  (xrel <= 10'd640) && (xrel[6:0] == 7'd0);   // 每128px主刻度
wire on_x_minor = (pix_y_d1 >= Y_MID) && (pix_y_d1 <= Y_MID + 2) &&
...
wire [1:0] tl0 = draw_char_at(pix_x_d1,pix_y_d1,X_LEFT+  0-4,tly,"0");
wire [1:0] tl2 = draw_char_at(pix_x_d1,pix_y_d1,X_LEFT+128-4,tly,"2");
wire [1:0] tl4 = draw_char_at(pix_x_d1,pix_y_d1,X_LEFT+256-4,tly,"4");
wire [1:0] tl6 = draw_char_at(pix_x_d1,pix_y_d1,X_LEFT+384-4,tly,"6");
wire [1:0] tl8 = draw_char_at(pix_x_d1,pix_y_d1,X_LEFT+512-4,tly,"8");
wire [1:0] tla = draw_char_at(pix_x_d1,pix_y_d1,X_LEFT+640-13,tly,"1");
wire [1:0] tlb = draw_char_at(pix_x_d1,pix_y_d1,X_LEFT+640-5,tly,"0");
wire on_tlabel = tl0[0]|tl2[0]|tl4[0]|tl6[0]|tl8[0]|tla[0]|tlb[0];

// "t/ms" 标签
wire [9:0] tlx = X_RIGHT - 10'd36;
wire [1:0] tl_t = draw_char_at(pix_x_d1,pix_y_d1,tlx,    Y_MID+10'd8,"t");
wire [1:0] tl_s = draw_char_at(pix_x_d1,pix_y_d1,tlx+9,  Y_MID+10'd8,"/");
wire [1:0] tl_m = draw_char_at(pix_x_d1,pix_y_d1,tlx+18, Y_MID+10'd8,"m");
wire [1:0] tl_ss= draw_char_at(pix_x_d1,pix_y_d1,tlx+27, Y_MID+10'd8,"s");
