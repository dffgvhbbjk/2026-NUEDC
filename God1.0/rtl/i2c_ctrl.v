...
//
// 注 (2026-08-01):
//   原样搬入野火官方 52_ov5640_vga_640x480 例程的 i2c_ctrl.v。
//   与自写 SCCB 状态机的关键差异:
//     1) ACK 检查: ACK 状态里采样 sda, 从机不应答 (ack==1) 则不推进,
//        状态机停住, 后续寄存器不再写 → config_done 拉不高 → 可诊断
//     2) 标准 START/STOP 时序: SDA 变沿发生在 SCL 低电平期间,
//        STOP 时 SDA 在 SCL 高电平期间拉高, 从机能正确识别事务边界
//   本板 50MHz 系统时钟, SCL = 250kHz。
//
    ...
    if(sys_rst_n == 1'b0)
        ...
        SEND_D_ADDR:
            if((cnt_bit == 3'd7) &&(cnt_i2c_clk == 3))
                ...
                state   <=  state;
        // 2026-08-01 修改: 去掉 ACK 检查, 应答时钟照走不卡死。
        //   原因: 官方控制器"从机不应答则停住", 在本板 SDA 应答信号临界时
        //   会卡死在第一个寄存器 (cfg_progress=0), 配置完全写不进去。
        //   I2C 从机接收数据不依赖主机验证 ACK, 照常收到写入。
        ACK_1:
            if(cnt_i2c_clk == 3)
                begin
                    if(addr_num == 1'b1)
